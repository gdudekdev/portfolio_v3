...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const bars = document.querySelectorAll(".navbar__bar");
  ...
  let observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        let navItem = document.querySelector(`[data-link="${sectionId}"]`);
        let color = navItem.getAttribute("data-color");

        // Reset all nav items
        navItems.forEach((item) => {
          item.classList.remove("active");
          item.querySelector("a").style.color = ""; // Reset color
          item.querySelector("a").classList.remove("textPulseWhite", "textPulseBlack"); // Reset animation
        });

        // Set active nav item
        navItem.classList.add("active");
        navItem.querySelector("a").style.color = color; // Set new color

        // Apply the correct animation based on the section
        if (sectionId === "hero" || sectionId === "projects" || sectionId === "contact") {
          navItem.querySelector("a").classList.add("textPulseBlack");
        } else {
          navItem.querySelector("a").classList.add("textPulseWhite");
        }

        // Change color of non-active items
        navItems.forEach((item) => {
          if (!item.classList.contains("active")) {
            if (sectionId === "hero") {
              ...
              item.querySelector("a").style.color = "gray"; // Set color for non-active items
              bars.forEach((element) => {
                element.style.backgroundColor = color; // Set bar color to match active item
              });
            }
          }
        });
